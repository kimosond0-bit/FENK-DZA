      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col my-auto max-h-[94vh]">
        ...
        <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-950 via-slate-900 to-teal-950 text-white flex items-center justify-between border-b border-slate-800 relative">
          ...
          {!isMandatory ? (
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-sky-500/20 border border-sky-400/30 text-sky-200 text-xs font-bold font-sans">
        ...
        {/* Mandatory Registration Banner when in mandatory mode */}
        {isMandatory && (
          <div className="px-4 py-2 bg-gradient-to-r from-sky-950 via-slate-900 to-emerald-950 border-b border-sky-800/40 text-xs text-sky-200 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <span>للدخول إلى منصة hakeDZ وتصفح منشورات الـ 69 ولاية، يرجى التسجيل أو تسجيل الدخول أولاً 🇩🇿</span>
            </div>
          </div>
        )}
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 text-right">
          ...
          {mode === 'login' && (
            <div className="space-y-4">
              ...
              {/* Quick Account Switcher (Filtered: Standard users only, Owner is strictly hidden and protected) */}
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
                <p className="text-xs font-bold text-slate-500 mb-2">أو التبديل السريع بين الحسابات المسجلة:</p>
                <div className="space-y-2 max-h-36 overflow-y-auto">
                  {availableUsers
                    .filter(u => u.role !== 'owner' && u.id !== 'usr_owner_kimo' && u.handle !== 'kimo_owner' && !u.phone?.includes('0777946398'))
                    .slice(0, 4)
                    .map((u) => (
                    <button
                      key={u.id}
